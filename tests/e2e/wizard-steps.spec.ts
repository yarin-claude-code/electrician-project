...
test('S7 — adding instrument creates row with type dropdown and date', async ({ page }) => {
  ...
  const dateInput = page.locator('input[type="date"], input[name*="date"], input[placeholder*="תאריך"]').first();
...
test('S8a — adding panel creates panel row and circuit table', async ({ page }) => {
  ...
  const panelInput = page.locator(
    'input[name*="panel"], input[placeholder*="לוח"], input[placeholder*="שם"]'
  ).first();
...
test('S10 — review step shows summary table', async ({ page }) => {
  ...
  const summary = page.locator(
    'table, [class*="summary"], [class*="review"]'
  ).first();
