...
const S = StyleSheet.create({
  ...
  // ── Status badge ──
  badgeGreen: { backgroundColor: '#16a34a', color: '#fff', borderRadius: 3, padding: '2pt 7pt', fontSize: 9, textAlign: 'center', alignSelf: 'flex-start' } as Style,
  badgeYellow: { backgroundColor: '#d97706', color: '#fff', borderRadius: 3, padding: '2pt 7pt', fontSize: 9, textAlign: 'center', alignSelf: 'flex-start' } as Style,
  badgeRed: { backgroundColor: '#dc2626', color: '#fff', borderRadius: 3, padding: '2pt 7pt', fontSize: 9, textAlign: 'center', alignSelf: 'flex-start' } as Style,
...
const INSTALLATION: Record<string, string> = {
  residential: 'מגורים', commercial: 'מסחרי', industrial: 'תעשייתי', other: 'אחר',
}
...
function InspectionPDF({ data }: { data: any }) {
  const { inspection, visualChecks, instruments, panels, faultLoop, defects, recommendations, generator } = data

  const approvalBadge =
    inspection.approval_status === 'approved' ? S.badgeGreen
    : inspection.approval_status === 'rejected' ? S.badgeRed
    : S.badgeYellow
  ...
  const CATEGORY_ORDER = ['general','cables_conductors','water_protection','fire_protection','grounding','height_compliance','tests_measurements','signage_labeling']
  ...
  return (
    <Document title={`דוח בדיקה - ${inspection.client_name}`} language="he">
      ...
      <Page size="A4" style={S.page}>
        ...
        <View style={S.noteBox}>
          <Text style={S.noteText}>
            דוח בדיקה זה מתבסס על מסמכים שנמסרו לבודק על ידי מזמין הבדיקה ומתכנן המתקן והם משקפים את מצב המתקן במועד הבדיקה.
            שינויים שיבוצעו במתקן החשמל לאחר מועד הבדיקה מחייבים בדיקה נוספת והם באחריות הבלעדית של מבצעי השינוי.
          </Text>
        </View>
          ...
          <View style={S.sectionBody}>
            <View style={S.row3}>
              ...
              <View style={S.cell}>
                ...
                {inspection.owner_phone ? <Text style={S.label}>{inspection.owner_phone}</Text> : null}
                {inspection.owner_email ? <Text style={S.label}>{inspection.owner_email}</Text> : null}
              </View>
              {/* Electrician */}
              <View style={S.cell}>
                <Text style={S.label}>החשמלאי המבצע</Text>
                <Text style={S.value}>{inspection.electrician_name || '—'}</Text>
                {inspection.electrician_phone ? <Text style={S.label}>{inspection.electrician_phone}</Text> : null}
                {inspection.electrician_license ? <Text style={S.label}>רישיון: {inspection.electrician_license}</Text> : null}
              </View>
              {/* Designer */}
              <View style={S.cell}>
                <Text style={S.label}>המתכנן</Text>
                <Text style={S.value}>{inspection.designer_name || '—'}</Text>
                {inspection.designer_phone ? <Text style={S.label}>{inspection.designer_phone}</Text> : null}
              </View>
            </View>
          </View>
        </View>
        ...
        {visualChecks.length > 0 && (
          <View style={S.section}>
            <Text style={S.sectionTitle}>ביקורת של המתקן ושל המסמכים הטכניים הנלווים ({visualChecks.length} פריטים)</Text>
            <View style={S.sectionBody}>
              {CATEGORY_ORDER.filter(catId => vcByCategory[catId]?.length).map((catId) => (
                <View key={catId} style={{ marginBottom: 6 }}>
                  <Text style={[S.label, { fontSize: 8, color: '#1B3A5C', marginBottom: 2, textAlign: 'right' }]}>
                  ...
                  <View style={S.table}>
                    ...
                    {vcByCategory[catId].map((vc: any, i: number) => (
                      <View key={i} style={i % 2 === 0 ? S.tRow : S.tRowAlt}>
                        ...
                        <Text style={[S.tCell, { flex: 1, textAlign: 'center' }, resultStyle(vc.result)]}>
              ...
              {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
              {visualChecks.filter((vc: any) => !CATEGORY_ORDER.includes(vc.category ?? 'general')).length > 0 && (
                <View style={{ marginBottom: 6 }}>
                  <View style={S.table}>
                    ...
                    {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                    {visualChecks.filter((vc: any) => !CATEGORY_ORDER.includes(vc.category || 'general')).map((vc: any, i: number) => (
                      <View key={i} style={i % 2 === 0 ? S.tRow : S.tRowAlt}>
                        <Text style={[S.tCell, { flex: 4 }]}>{vc.item_label || vc.item_key}</Text>
                        <Text style={[S.tCell, { flex: 1, textAlign: 'center' }, resultStyle(vc.result)]}>
                          {resultText(vc.result)}
              ...
              {/* Visual check photos */}
              {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
              {visualChecks.filter((vc: any) => vc.photo_url && vc.photo_url.startsWith('http')).length > 0 && (
                <View style={{ marginTop: 6 }}>
                  <Text style={[S.label, { marginBottom: 4, color: '#1B3A5C' }]}>תמונות מהבדיקה החזותית</Text>
                  <View style={{ flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 6 }}>
                    {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                    {visualChecks.filter((vc: any) => vc.photo_url && vc.photo_url.startsWith('http')).map((vc: any, i: number) => (
                      <View key={i} style={{ alignItems: 'center', width: 100 }}>
                        {/* eslint-disable-next-line jsx-a11y/alt-text */}
                        <Image src={vc.photo_url} style={{ width: 100, height: 75, objectFit: 'cover', borderRadius: 3 }} />
                        <Text style={[S.label, { textAlign: 'center', marginTop: 2, fontSize: 7 }]}>{vc.item_label || vc.item_key}</Text>
                      </View>
                    ))}
                  </View>
                </View>
      ...
      <Page size="A4" style={S.page}>
            ...
            {instruments.length === 0 ? (
              ...
              <View style={S.table}>
                ...
                {instruments.map((inst: any, i: number) => (
                  <View key={i} style={i % 2 === 0 ? S.tRow : S.tRowAlt}>
                    <Text style={[S.tCell, { flex: 3 }]}>{inst.name || inst.instrument_type || '—'}</Text>
        ...
        {/* Panels */}
        {panels.length > 0 && panels.map((panel: any, pi: number) => { // eslint-disable-line @typescript-eslint/no-explicit-any
          const circuits = Array.isArray(panel.circuit_measurements) ? panel.circuit_measurements : []
          const pfLabel = (v: string | null) => v === 'pass' ? 'תקין' : v === 'fail' ? 'ליקוי' : v === 'na' ? 'נ/ר' : '—'
          return (
            <View key={pi} style={S.section}>
              <Text style={S.sectionTitle}>תוצאות הבדיקה של לוח: {panel.panel_name || panel.name || `לוח ${pi + 1}`}</Text>
              <View style={S.sectionBody}>
                {circuits.length > 0 && (
                  <View style={S.table}>
                    <View style={S.tHeaderRow}>
                    ...
                    {circuits.map((c: any, ci: number) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
                      <View key={ci} style={ci % 2 === 0 ? S.tRow : S.tRowAlt}>
                        ...
                        <Text style={[S.tCell, { flex: 1.5 }]}>{c.grounding_continuity ?? '—'}</Text>
                        <Text style={[S.tCell, { flex: 1.5 }]}>{c.fault_loop_impedance ?? '—'}</Text>
                        <Text style={[S.tCell, { flex: 1.5 }]}>{c.voltage_1l_n ?? '—'}</Text>
                        <Text style={[S.tCell, { flex: 1 }]}>{pfLabel(c.overcurrent_protection)}</Text>
                ...
                {circuits.length === 0 && (
                  <Text style={S.label}>לא הוזנו מדידות</Text>
                )}
              </View>
            </View>
          )
              ...
              <View style={S.table}>
                ...
                {faultLoop.z_1ph_2ph != null && (
                  <View style={S.tRow}>
                    ...
                    <Text style={[S.tCell, { flex: 2 }]}>{(400 / parseFloat(String(faultLoop.z_1ph_2ph))).toFixed(1)}</Text>
                    <Text style={[S.tCell, { flex: 2 }]}>—</Text>
                  </View>
                )}
              </View>
                ...
                <View style={[S.table, { marginBottom: 4 }]}>
                  ...
                  {generator.docReviews.map((vc: any, i: number) => (
                    <View key={i} style={i % 2 === 0 ? S.tRow : S.tRowAlt}>
                      ...
                      <Text style={[S.tCell, { flex: 1, textAlign: 'center' }, resultStyle(vc.result)]}>{resultText(vc.result)}</Text>
                      <Text style={[S.tCell, { flex: 2 }]}>{vc.notes || ''}</Text>
                    </View>
                  ))}
                </View>
          ...
          <View style={S.sectionBody}>
            {defects.length === 0 ? (
              ...
              <View>
                <View style={S.table}>
                  ...
                  {defects.map((d: any, i: number) => (
                    <View key={i} style={i % 2 === 0 ? S.tRow : S.tRowAlt}>
                      ...
                      <Text style={[S.tCell, { flex: 1, textAlign: 'center' }, d.severity === 'critical' ? S.failText : {}]}>
                        {severityLabel(d.severity)}
                      </Text>
                      <Text style={[S.tCell, { flex: 2 }]}>{fmt(d.target_repair_date) || '—'}</Text>
                      <Text style={[S.tCell, { flex: 1, textAlign: 'center' }, d.resolved ? S.passText : {}]}>
                        {d.resolved ? 'כן' : 'לא'}
                      </Text>
                    </View>
                  ))}
                ...
                {/* Defect photos */}
                {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                {defects.filter((d: any) => d.photo_url && d.photo_url.startsWith('http')).length > 0 && (
                  <View style={{ marginTop: 6 }}>
                    <Text style={[S.label, { marginBottom: 4, color: '#1B3A5C' }]}>תמונות ליקויים</Text>
                    <View style={{ flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 6 }}>
                      {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                      {defects.filter((d: any) => d.photo_url && d.photo_url.startsWith('http')).map((d: any, i: number) => (
                        <View key={i} style={{ alignItems: 'center', width: 100 }}>
                          <Image src={d.photo_url} style={{ width: 100, height: 75, objectFit: 'cover', borderRadius: 3 }} />
                          <Text style={[S.label, { textAlign: 'center', marginTop: 2 }]}>ליקוי {i + 1}</Text>
                        </View>
                      ))}
                    </View>
                  </View>
            ...
            <View style={S.signatureRow}>
              <View style={S.signatureBlock}>
                ...
                {inspection.inspector_signature ? (
                  <Image src={inspection.inspector_signature} style={{ width: '100%', height: 50 }} />
                ) : (
                  <View style={S.signatureBox} />
                )}
                <View style={S.signatureLine} />
                <Text style={S.signatureLabel}>תאריך: {fmt(inspection.submitted_at || inspection.updated_at)}</Text>
              </View>
              <View style={S.signatureBlock}>
...
export async function POST(
  ...
  if (!supabaseUrl?.startsWith('http')) {
    const demoData = {
      ...
      visualChecks: [
        // כללי
        { category: 'general', item_key: 'gen_panel_conformity',      item_label: 'התאמת הלוח והציוד שבו לתקנים',                    result: 'pass', notes: '' },
        { category: 'general', item_key: 'gen_plan_conformity',        item_label: 'התאמה לתוכניות',                                   result: 'pass', notes: '' },
        { category: 'general', item_key: 'gen_diagram_nameplate',      item_label: 'תרשים הלוח בצמוד ללוחית הלוח',                    result: 'pass', notes: '' },
        { category: 'general', item_key: 'gen_access_ventilation',     item_label: 'גישה נוחה, תנאי אוורור, אור, מיקום הלוח',         result: 'pass', notes: '' },
        { category: 'general', item_key: 'gen_stable_surface',         item_label: 'קביעה על משטחי יציב',                             result: 'pass', notes: '' },
        { category: 'general', item_key: 'gen_spacing_1_75',           item_label: 'מרווחים בין שני סוגי אספקה פחות מ-1.75 ס"מ',      result: 'pass', notes: '' },
        // כבלים ומוליכים
        { category: 'cables_conductors', item_key: 'cable_fuse_conductor_match', item_label: 'התאמת המבטחים לשטחי המוליכים',          result: 'pass', notes: '' },
        { category: 'cables_conductors', item_key: 'cable_neutral_ground_screw', item_label: 'מוליכי אפס והארקה כל אחד בבורג נפרד',  result: 'pass', notes: '' },
        { category: 'cables_conductors', item_key: 'cable_color_coding',         item_label: 'צבעי זיהוי המוליכים (הצבע הכחול)',       result: 'pass', notes: '' },
        { category: 'cables_conductors', item_key: 'cable_entry_protection',     item_label: 'כניסת הכבלים דרך פתח מוגן למניעת פציעות', result: 'pass', notes: '' },
        { category: 'cables_conductors', item_key: 'cable_circuit_separation',   item_label: 'הפרדה בין מעגלים',                       result: 'pass', notes: '' },
        { category: 'cables_conductors', item_key: 'cable_bending_angles',       item_label: 'זוויות כיפוף מוליכים בלוח',              result: 'pass', notes: '' },
        { category: 'cables_conductors', item_key: 'cable_ties_connections',     item_label: 'תקינות מעלי כבל וחיבורים',               result: 'pass', notes: '' },
        // מים
        { category: 'water_protection', item_key: 'water_no_pipe_proximity', item_label: 'אין קרבה לצנרת מים, גז',                    result: 'pass', notes: '' },
        { category: 'water_protection', item_key: 'water_sealing_covers',    item_label: 'איטומים/כיסויים למניעת חדירת מים',          result: 'pass', notes: '' },
        { category: 'water_protection', item_key: 'water_fire_sealing',      item_label: 'איטום למניעת הפצות אש',                     result: 'pass', notes: '' },
        // אש
        { category: 'fire_protection', item_key: 'fire_detection_trip',    item_label: 'קיום מערכת גילוי/כיבוי אש וחיבור לסליל הפסקה', result: 'na',   notes: '' },
        { category: 'fire_protection', item_key: 'fire_resistant_cables',  item_label: 'כבלים חסיני אש (במידת הצורך)',                result: 'pass', notes: '' },
        { category: 'fire_protection', item_key: 'fire_panel_base',        item_label: 'מסד הלוח מוארק',                              result: 'pass', notes: '' },
        // הארקה
        { category: 'grounding', item_key: 'gnd_panel_door',          item_label: 'דלת הלוח מוארקת (כשמותקן עליה ציוד)',              result: 'pass', notes: '' },
        { category: 'grounding', item_key: 'gnd_metallic_continuity', item_label: 'רציפות הארקה לשירותים מתכתיים',                    result: 'pass', notes: '' },
        { category: 'grounding', item_key: 'gnd_ct_grounding',        item_label: 'הארקת שנאי זרם',                                  result: 'na',   notes: '' },
        // גובה
        { category: 'height_compliance', item_key: 'height_operating_devices', item_label: 'גובה אמצעי הפעלה (במתקן דירתי מעל 1.4 מ\')', result: 'pass', notes: '' },
        { category: 'height_compliance', item_key: 'height_terminals',         item_label: 'גובה מחדקי חיבורים',                      result: 'pass', notes: '' },
        // בדיקות מדידות
        { category: 'tests_measurements', item_key: 'test_disconnect_capacity', item_label: 'כושר ניתוק – התאמה לזרם קצר צפוי',       result: 'pass', notes: '' },
        { category: 'tests_measurements', item_key: 'test_trip_coil',           item_label: 'פעולת סליל הפסקה – Trip coil',            result: 'na',   notes: '' },
        { category: 'tests_measurements', item_key: 'test_ins_supply',          item_label: 'התנגדות בידוד כבל ההזנה',                 result: 'pass', notes: '' },
        { category: 'tests_measurements', item_key: 'test_ins_branch',          item_label: 'התנגדות בידוד כבלים למעגלים',             result: 'pass', notes: '' },
        { category: 'tests_measurements', item_key: 'test_rcd_condition',       item_label: 'תקינות מפסקי מגן בזרם דלף',              result: 'pass', notes: '' },
        // שילוט
        { category: 'signage_labeling', item_key: 'sign_supply_source',        item_label: 'שילוט מקור ההזנה',                        result: 'pass', notes: '' },
        { category: 'signage_labeling', item_key: 'sign_panel_purpose',        item_label: 'שילוט מטרת הלוח (מ"מ)',                   result: 'pass', notes: '' },
        { category: 'signage_labeling', item_key: 'sign_breaker_designations', item_label: 'שילוט ייעוד המפסקים (בר-קיימא)',           result: 'fail', notes: 'חסר שילוט על 3 מפסקים' },
        { category: 'signage_labeling', item_key: 'sign_cable_labeling',       item_label: 'שילוט ייעוד הכבלים',                      result: 'pass', notes: '' },
        { category: 'signage_labeling', item_key: 'sign_terminals_100a',       item_label: 'סימון מחדקים (בלוחות מעל 100A)',           result: 'na',   notes: '' },
        { category: 'signage_labeling', item_key: 'sign_door_breakers',        item_label: 'שילוט שמות המפסקים על דלת הלוח',          result: 'pass', notes: '' },
        { category: 'signage_labeling', item_key: 'sign_spare_breakers',       item_label: 'מפסקים שמורים ללא שילוט במצב מחובר',      result: 'pass', notes: '' },
      ],
      instruments: [
        { name: 'מגה-אוהמטר (Megohmmeter)', model: 'Fluke 1587', serial_number: 'SN-001', calibration_date: '2025-06-01' },
        { name: 'בודק לולאת תקלה (Loop Tester)', model: 'Metrel MI3102', serial_number: 'SN-002', calibration_date: '2025-06-01' },
      ],
      panels: [
        {
          panel_name: 'לוח ראשי',
          circuit_measurements: [
            { circuit_number: 1, ins_1l_e: 200, grounding_continuity: 0.3, fault_loop_impedance: 0.45, voltage: 230 },
            { circuit_number: 2, ins_1l_e: 185, grounding_continuity: 0.4, fault_loop_impedance: 0.48, voltage: 230 },
            { circuit_number: 3, ins_1l_e: 210, grounding_continuity: 0.3, fault_loop_impedance: 0.42, voltage: 230 },
          ],
        },
      ],
      generator: {
        cert: { manufacturer: 'Perkins', model: '1104A-44TG2', serial_number: 'GEN-001', power_rating: '80 kVA', installation_location: 'חדר גנרטור', panel_location: 'לוח חשמל ראשי', supplier: 'אנרגיה בע"מ', permit_number: 'P-2025-001' },
        docReviews: [
      ...
      faultLoop: {
        zph1_n: '0.35', zph1_e: '0.41',
        zph2_n: '0.37', zph2_e: '0.42',
        zph3_n: '0.36', zph3_e: '0.40',
      },
      defects: [
        { description: 'חסר שילוט על 3 מפסקים בלוח הראשי', severity: 'minor', resolved: false, fix_deadline: '2026-03-15' },
      ],
      recommendations: [
  ...
  try {
    ...
    const [
      ...
      supabase.from('panels').select('*, circuit_measurements(*)').eq('inspection_id', id).order('sort_order'),
    ...
    if (genCert) {
      const [{ data: docReviews }, { data: genVisualChecks }] = await Promise.all([
        supabase.from('generator_doc_reviews').select('*').eq('certificate_id', genCert.id).order('created_at'),
        supabase.from('generator_visual_checks').select('*').eq('certificate_id', genCert.id).order('created_at'),
      ])
      generator = { cert: genCert, docReviews: docReviews ?? [], visualChecks: genVisualChecks ?? [] }
    }
    ...
    try {
      ...
      const { data: uploadData } = await supabase.storage
        .from('photos').upload(fileName, pdfBuffer, { contentType: 'application/pdf', upsert: true })
      if (uploadData) {
