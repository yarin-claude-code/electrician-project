  ...
  useEffect(() => {
    const loadRecommendations = async (): Promise<void> => {
      const { data } = (await supabase
        ...
        .order('sort_order')) as { data: Recommendation[] | null }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={add} size="sm" className="gap-2 bg-blue-900 hover:bg-blue-800">
      ...
      {recommendations.length === 0 ? (
        ...
        <div className="space-y-4">
          {recommendations.map((rec, idx) => (
            <div key={rec.id} className="space-y-4 rounded-lg border bg-white p-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">המלצה {idx + 1}</span>
                <Button
                  variant="ghost"
                  size="sm"
