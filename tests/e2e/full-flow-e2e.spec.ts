...
test('S14 Phase 4 — fill general info fields and advance', async ({ page }) => {
  ...
  // Fill client name
  const clientInput = page.locator('#clientName, input[name*="client"], input[placeholder*="שם לקוח"]').first();
  ...
  // Fill address
  const addressInput = page.locator('input[name*="address"], input[placeholder*="כתובת"], #address').first();
...
test('S14 Phase 11 — review step: no Next, has summary and canvas', async ({ page }) => {
  ...
  const nextVisible = await page.locator('button:has-text("הבא")').isVisible().catch(() => false);
