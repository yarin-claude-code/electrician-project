...
test('S11 — Visual Theme', async ({ page }) => {
  ...
  console.log(`  No horizontal overflow at 375px: ${noHorizontalOverflow} (scrollWidth=${scrollWidth}, clientWidth=${clientWidth})`);
...
test('S1 — Auth', async ({ page }) => {
  ...
  if (submitVisible) {
    ...
    await page.waitForTimeout(500);
    const errorMessages = await page.locator('[class*="error"], [class*="invalid"], [aria-invalid="true"] + *, p[class*="text-red"], p[class*="destructive"]').count();
    const anyErrorText = await page.locator('text=/שדה|נדרש|חובה|אימייל|סיסמ/').count();
    console.log(`S1: Validation errors found: ${errorMessages} error elements, ${anyErrorText} Hebrew error texts`);
  }
...
test('S5 — Wizard Step 1 General Info', async ({ page }) => {
  ...
  // Check fields
  const fieldChecks = [
    { label: 'client name', selector: 'input[name*="client"], input[placeholder*="שם"], input[id*="client"]' },
    { label: 'address', selector: 'input[name*="address"], input[placeholder*="כתובת"], input[id*="address"]' },
...
test('Toggle and Radio Visual Check', async ({ page }) => {
  ...
  if (await nextBtn.isVisible()) {
    ...
    const passBtns = await page.locator('button:has-text("תקין"), button:has-text("עובר"), button:has-text("Pass")').count();
    const failBtns = await page.locator('button:has-text("לא תקין"), button:has-text("נכשל"), button:has-text("Fail")').count();
    const naBtns = await page.locator('button:has-text("לא רלוונטי"), button:has-text("N/A"), button:has-text("ל/ר")').count();

    console.log(`Toggle check step 2: Pass buttons: ${passBtns}, Fail buttons: ${failBtns}, N/A buttons: ${naBtns}`);
