...
export const DEMO_INSPECTIONS: Inspection[] = [
  {
    ...
    electrician_phone: null,
    electrician_email: null,
    designer_name: null,
    designer_phone: null,
    designer_email: null,
    has_generator: false,
  ...
  },
  {
    id: '3',
    inspector_id: 'dev',
    client_name: 'מרכז קניות',
    installation_type: 'commercial',
    address: 'שדרות בן גוריון 5, ירושלים',
    connection_size_amps: 400,
  ...
  },
  {
    ...
    client_name: 'דוד כהן',
    installation_type: 'residential',
    address: 'רחוב ויצמן 3, רמת גן',
    connection_size_amps: 63,
  ...
  },
  {
    id: '5',
    inspector_id: 'dev',
    client_name: 'בניין משרדים',
    installation_type: 'commercial',
    address: 'אחד העם 20, תל אביב',
    connection_size_amps: 250,
...
export const buildStatusData = (inspections: Inspection[]): StatusDataItem[] =>
  [
    {
      name: 'מאושר',
      value: inspections.filter((i) => i.approval_status === 'approved').length,
      color: COLOR_APPROVED,
    },
    {
      name: 'עם המלצות',
      value: inspections.filter((i) => i.approval_status === 'approved_with_recommendations')
        .length,
      color: COLOR_WITH_RECS,
    },
    {
      name: 'נדחה',
      value: inspections.filter((i) => i.approval_status === 'rejected').length,
      color: COLOR_REJECTED,
    },
    {
      name: 'בתהליך',
      value: inspections.filter((i) => i.status === 'draft' || i.status === 'in_progress').length,
